...
def test_eligible_history_only_drops_this_runs_own_same_day_picks():
    """A manually-seeded real entry can share `first_shown` with the
    `monday` being regenerated by coincidence (China's entry does, for
    2026-09-14 - the exact event this whole feature was built to stop from
    resurfacing). Only an entry this function itself wrote for that same
    `monday`, marked with `generated_for_monday`, may be excluded; date
    equality alone must never be enough."""
    monday = "2026-09-14"
    manually_seeded = {"first_shown": monday, "name": "Flood in China"}
    auto_written = {"first_shown": monday, "name": "x",
                    "generated_for_monday": monday}
    history = {"1104081": manually_seeded, "2": auto_written}

    eligible = report._eligible_history(history, monday)

    assert "1104081" in eligible, "manually-seeded entry wrongly excluded"
    assert "2" not in eligible, "this run's own earlier same-day pick was not excluded"
